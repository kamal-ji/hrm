...
<head>
    ...
    <title>Login |{{get('name')}} - Verify Otp</title>
...
<body class="bg-white">
                                ...
                                <div class="card border-0 p-lg-3 shadow-lg rounded-2">
                                    <div class="card-body">
                                        <div class="text-center mb-3">
                                            <h5 class="mb-2">Verify your Otp</h5>
                                        ...
                                        <div class="mb-1 position-relative">
                                            <button type="submit"
                                                ...
                                                id="verifyotp"><span id="resettext">Verify </span> <!-- Spinner (Initially hidden) -->
                                                <div id="loginSpinner"
                                        ...
                                        <div class="text-center">
                                            <h6 class="fw-normal fs-14 text-dark mb-0">Return to
                                                <a href="javascript:void(0)" id="resendOtpLink" class="hover-a"> Resent
                                                    Otp?</a> |
                                                <a href="{{route('forgot-password')}}" class="hover-a"> Use different
                                                    email</a>
                                            </h6>
                                        </div>
                                    </div><!-- end card body -->
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Auto-tab between OTP digits (if you have multiple inputs)
    ...
    $(document).ready(function() {
        ...
        $('#apiforgotForm').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('verifyotp.submit') }}", // Use the route name instead of the URL route('admin.login.submit'),
                method: 'POST',
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (response.attempts_left) {
                            errorMsg += ` Attempts left: ${response.attempts_left}`;
                        }
                ...
                error: function(xhr) {
                    let errorMsg = 'Otp verfication failed';
                    if (xhr.responseJSON && xhr.responseJSON.message) {
                        errorMsg = xhr.responseJSON.message;

                        if (xhr.responseJSON.attempts_left) {
                            errorMsg += ` Attempts left: ${xhr.responseJSON.attempts_left}`;
                        }
                    }
                    console.log("Generated error message:", errorMsg);
                    showError(errorMsg);
                }
            });
        });
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        .message); // You can replace this with your own toast or alert logic
                    } else {
                        let errorMsg = response.message || 'Failed to resend OTP.';
                        if (response.attempts_left) {
                            errorMsg += ` Attempts left: ${response.attempts_left}`;
                        }
                        showError(errorMsg);
