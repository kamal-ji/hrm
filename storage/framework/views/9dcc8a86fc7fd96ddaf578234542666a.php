    ...
    <div class="content content-two">
        ...
        <div class="d-flex d-block align-items-center justify-content-between flex-wrap gap-3 mb-3">
            ...
            <div class="d-flex my-xl-auto right-content align-items-center flex-wrap gap-2">
                <div>
                    <a href="<?php echo e(route('members.create')); ?>" class="btn btn-primary d-flex align-items-center">
        ...
        <!-- Table Search Start -->
        <div class="mb-3">
            <form id="filter-form" method="GET" action="<?php echo e(route('members.index')); ?>" class="mb-4">
                <div class="row">
                    <div class="col-md-3">
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#member-table').DataTable({
                ...
                autoWidth: false,
                ajax: {
                    url: "<?php echo e(route('members.index')); ?>",
                    type: 'GET',
                    data: function(d) {
    ...
    { data: 'sponsor' },   // Add this
            ...
            $('#bulk-approve').on('click', function() {
                ...
                if (confirm('Are you sure you want to approve ' + selectedIds.length + ' member(s)?')) {
                    $.ajax({
                        url: "<?php echo e(route('members.bulk.approve')); ?>",
                        type: 'POST',
                        data: {
                            ids: selectedIds
                        },
       ...
       // Toggle member status (activate/deactivate)
        function toggleStatus(id, status) {
            ...
            if (confirm('Are you sure you want to ' + action + ' this member?')) {
                $.ajax({
                    url: "<?php echo e(url('admin/members')); ?>/" + id + "/status",
                    type: 'PUT',
                    data: {
                        status: status
                    },
        ...
        $(document).on('click', '.btn-approve', function() {
            ...
            if (confirm('Are you sure you want to approve this member?')) {
                $.ajax({
                    url: "<?php echo e(url('admin/members')); ?>/" + id + "/approve",
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
        ...
        $(document).on('click', '.btn-reject', function() {
            ...
            if (confirm('Are you sure you want to reject this member?')) {
                $.ajax({
                    url: "<?php echo e(url('admin/members')); ?>/" + id + "/reject",
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
        ...
        function deleteMember(id) {
            if (confirm('Are you sure you want to delete this member?')) {
                $.ajax({
                    url: "<?php echo e(url('admin/members')); ?>/" + id,
                    type: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\wamp64\www\hrm\resources\views/backend/members/index.blade.php ENDPATH**/ ?>