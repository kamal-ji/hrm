        ...
        <div class="row">
            <div class="col-md-10 mx-auto">
                <div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <h5 class="mb-3">Edit Holiday Template</h5>
                            <form action="{{ route('holiday-template.update', $template->id) }}" id="createForm" enctype="multipart/form-data">
                                ...
                                <div class="row gx-3">
                                    ...
                                    <div class="col-lg-6 col-md-6">
                                        <div class="mb-2">
                                            ...
                                            <input type="number" min="2000" max="2100" class="form-control" name="year" id="year" value="{{ $template->selected_year }}" onchange="updateDateRange()">
                                        </div>
                                    </div>
    ...
    <script>
                    ...
                    success: function(response) {
                        if (response.success) {
                            showSuccess(response.message ||
                                'Holiday Template updated successfully');
        ...
        function addHoliday(data = {}) {
            jQuery('[xrole="holidays"]').append(`<div class="input-group mb-2" xrole="holiday">
                <input type="text" name="holiday_dates[${holidayCounter}][name]" class="form-control" placeholder="Holiday Name" value="${data.name || ''}">
                <input type="date" name="holiday_dates[${holidayCounter}][date]" xrole="holiday-date" class="form-control" placeholder="Date" value="${data.date || ''}">
        ...
        @foreach($template->dates ?? [] as $holiday)
            addHoliday({
                name: '{{ $holiday->name }}',
                date: '{{ $holiday->selected_date }}'
            });
        @endforeach

        @if(empty($template->dates))
            addHoliday();
        @endif
