...
@section('content')
    ...
    <div class="content content-two">
        ...
        <!-- Table Search Start -->
        <div class="mb-3">
            <form id="filter-form" method="GET" action="{{ route('employees.index') }}" class="mb-4">
                <div class="row">
                    <div class="col-md-3">
    ...
    { data: 'sponsor' },   // Add this
            ...
            $('#bulk-approve').on('click', function() {
                ...
                if (confirm('Are you sure you want to approve ' + selectedIds.length + ' employee(s)?')) {
                    $.ajax({
                        url: "{{ route('employees.bulk.approve') }}",
                        type: 'POST',
                        data: {
                            ids: selectedIds
                        },
       ...
       // Toggle employee status (activate/deactivate)
        function toggleStatus(id, status) {
            ...
            if (confirm('Are you sure you want to ' + action + ' this employee?')) {
                $.ajax({
                    url: "{{ url('admin/employees') }}/" + id + "/status",
                    type: 'PUT',
                    data: {
                        status: status
                    },
        ...
        $(document).on('click', '.btn-approve', function() {
            ...
            if (confirm('Are you sure you want to approve this employee?')) {
                $.ajax({
                    url: "{{ url('admin/employees') }}/" + id + "/approve",
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
        ...
        $(document).on('click', '.btn-reject', function() {
            ...
            if (confirm('Are you sure you want to reject this employee?')) {
                $.ajax({
                    url: "{{ url('admin/employees') }}/" + id + "/reject",
                    type: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
